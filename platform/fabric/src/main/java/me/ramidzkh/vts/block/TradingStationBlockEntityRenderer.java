...
public class TradingStationBlockEntityRenderer implements BlockEntityRenderer<TradingStationBlockEntity> {
    ...
    @Override
    public void render(TradingStationBlockEntity tradingStation, float partialTicks, PoseStack matrices, MultiBufferSource multiBufferSource, int light, int overlay) {
        ...
        float[] angles = new float[variants.size()];

        for (int i = 0; i < angles.length; i++) {
            angles[i] = i * 360F / variants.size();
        }

        float time = ((int) tradingStation.getLevel().getGameTime()) + partialTicks;
